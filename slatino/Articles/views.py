...
def article_last(request):
    articles = Article.objects.filter(published=True, post_type='article').order_by('order').order_by('-pub_date')[:article_last_count]
    return render_to_response("Articles/list.html", RequestContext(request,
                              dict(articles=articles, title=_(u"Last Articles"))))


def article_list(request):
    articles = Article.objects.filter(published=True, post_type='article').order_by('order').order_by('-pub_date')
...
def article_view(request, article_slug):
    article = get_object_or_404(Article, slug=article_slug, post_type='article')
    return render_to_response("Articles/one.html", RequestContext(request,
                              dict(article=article)))


def past_image(request, article_id):
    article = get_object_or_404(Article, id=article_id, post_type='article')
